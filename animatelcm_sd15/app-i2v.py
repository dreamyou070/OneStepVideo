...
class AnimateController:
    def __init__(self):
        ...
        self.inference_config = OmegaConf.load("configs/inference-i2v.yaml")
    ...
    def animate(
        ...
        print(self.inference_config.noise_scheduler_kwargs["beta_end"])
        self.inference_config.noise_scheduler_kwargs["beta_end"] = beta_end_slider
        
        self.unet.img_encoder.motion_scale = motion_scale_slider
...
def ui():
    with gr.Blocks(css=css) as demo:
        gr.Markdown(
            ...
            '''
            Important Notes: 
            1. The generation speed is around few seconds. 
            2. Increase the sampling step for better sample quality.
            '''
        )
        with gr.Column(variant="panel"):
            with gr.Row():
                image_upload = gr.Image(label="Upload Image", tool="select", type="filepath")

                base_model_dropdown = gr.Dropdown(
                    label="Select base Dreambooth model (required)",
                    choices=controller.personalized_model_list,
                    interactive=True,
                    value="realistic1.safetensors"
                )
        ...
        with gr.Column(variant="panel"):
            ...
            prompt_textbox = gr.Textbox(label="Prompt", lines=2, value="best quality")
            ...
            with gr.Row().style(equal_height=False):
                with gr.Column():
                    ...
                    motion_scale_slider = gr.Slider(
                        label="motion scale (better identity with smaller scale)", value=0.8, minimum=0.0, maximum=1.5, step=0.05
                    )
                    beta_end_slider = gr.Slider(
                        label="beta end (a tricky way for selecting noisy steps)", value=0.014, minimum=0.012, maximum=0.016, step=0.001)
            ...
            examples = [
                [0.8, 0.8, "good quality, cloud", "bad quality", "LCM", 4, 768, 16, 512, 1, 1234, "test_imgs/cloud.jpeg", 0.014, 0.8],
                [0.8, 0.8, "good quality, dog", "bad quality", "LCM", 4, 768, 16, 512, 1, 1234, "test_imgs/dog.jpg", 0.014, 0.8],
                [0.8, 0.8, "good quality, fire", "bad quality", "LCM", 4, 768, 16, 512, 1, 1234, "test_imgs/fire.jpg", 0.014, 0.8],
                [0.8, 0.8, "good quality, fox, snow", "bad quality", "LCM", 4, 768, 16, 512, 1, 1234, "test_imgs/fox.jpg", 0.014, 0.8],
                [0.8, 0.8, "good quality, girl, wind, flower", "bad quality", "LCM", 4, 768, 16, 512, 1, 1235, "test_imgs/girl_flower.jpg", 0.014, 0.8],
                [0.8, 0.8, "good quality, lighter, fire", "bad quality", "LCM", 4, 768, 16, 512, 1, 1235, "test_imgs/lighter.jpg", 0.014, 0.8],
                [0.8, 0.8, "good quality, snowman, fire", "bad quality", "LCM", 4, 768, 16, 512, 1, 1234, "test_imgs/snow_man_fire.jpg", 0.014, 0.8],
            ]
            gr.Examples(
                examples = examples,
                inputs=[
