...
def resize_and_crop(image_path, target_width, target_height):
    ...
    if target_ratio > orig_ratio:
        ...
        resize_height = round((target_width / orig_width) * orig_height)
    else:
        resize_height = target_height
        resize_width = round((target_height / orig_height) * orig_width)
    
    resized_image = image.resize((resize_width, resize_height), Image.LANCZOS)
    
    x0 = (resize_width - target_width) / 2
    y0 = (resize_height - target_height) / 2
    x1 = x0 + target_width
    y1 = y0 + target_height
...
def adjust_colors(src_img, target_img):
    src_hsv = cv2.cvtColor(src_img, cv2.COLOR_BGR2HSV)
    target_hsv = cv2.cvtColor(target_img, cv2.COLOR_BGR2HSV)

    for idx in range(3):
        src_mean = np.mean(src_hsv[:, :, idx])
        target_mean = np.mean(target_hsv[:, :, idx])

        diff = src_mean - target_mean

        if idx == 0:
            target_hsv[:, :, idx] = np.clip(target_hsv[:, :, idx] + 0.03 * diff, 0, 180)
        else:  
            target_hsv[:, :, idx] = np.clip(target_hsv[:, :, idx] + 1.0 * diff, 0, 255)

    adjusted_img = cv2.cvtColor(target_hsv, cv2.COLOR_HSV2BGR)
    return adjusted_img
...
def load_weights(
    ...
    if motion_module_path != "":
        ...
        ext = os.path.splitext(motion_module_path)[-1]
