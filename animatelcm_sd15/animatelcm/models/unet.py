...
# from .adapter import PixelAdapter # Not ready
...
class UNet3DConditionModel(ModelMixin, ConfigMixin):
    ...
    def __init__(
        ...
        self.img_encoder = Adapter(cin=4, channels=[320, 640, 1280, 1280][:4], nums_rb=2, ksize=1, sk=True, use_conv=False) if use_img_encoder else None
    ...
    def forward(
        ...
        for downsample_block in self.down_blocks:
            ...
            added_feature = img_features[img_feature_idx] if img_features is not None else torch.tensor(0.).to(sample.device, sample.dtype)
            added_feature = added_feature + ctrl_features[img_feature_idx] if ctrl_features is not None else added_feature
    ...
    def from_pretrained_2d(cls, pretrained_model_path, subfolder=None, unet_additional_kwargs=None):
        ...
        if os.path.splitext(model_file)[-1] == '.safetensors':
            state_dict = {}
