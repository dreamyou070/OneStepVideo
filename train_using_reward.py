...
def main(image_finetune: bool,
    ...
    train_dataset = WebVid10M(
        csv_path=r'/share0/dreamyou070/dreamyou070/MyData/video/webvid-10M/webvid-10M-csv/0000_1.csv',
    ...
    for epoch in range(first_epoch, num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            output = validation_pipeline(prompt=prompt,
                                         ...
                                         num_frames=1,  # length = 48
                                         guidance_scale=guidance_scale,
                                         num_inference_steps=inference_step,
                                         generator=torch.Generator("cpu").manual_seed(0),
                                         motion_controller=student_motion_controller,
                                         window_size=window_size, ).frames[0]
            image_quality_score = reward_function(output)
            image_qualit_loss = 1 - image_quality_score
            loss += image_qualit_loss
            # ----------------------------------------------------------------------------------------------------------------------------------
