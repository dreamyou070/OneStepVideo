...
def main(args):
    ...
    print(f' step 1. logging dir and accelerator')
    ...
    if args.report_to == "wandb":
        ...
        wandb.init(project=args.wandb_project,
                   name=args.wandb_run_name, config=args)

    print(f' step 2. Make one log on every process')
    ...
    print(f' step 3. set seed')
    if args.seed is not None:
        set_seed(args.seed)

    print(f' step 4. save dir')
    ...
    print(f' step 5. Create the noise scheduler and the desired noise schedule')
    ...
    print(f' (5.3) tokenizers, text_encoder, vae, teacher_unet')
    ...
    print(f' (5.4) Freeze teacher vae, text_encoder, and teacher_unet')
    vae.requires_grad_(False)
    text_encoder.requires_grad_(False)
    teacher_unet.requires_grad_(False)
    print(f' (5.5) Create online student U-Net')
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(unet):
                ...
                # ----------------------------------------------------------------------------
                # 2. text prompt
                encoded_text = compute_embeddings_fn(text) # prompt_embed
                pixel_values = image.to(dtype=weight_dtype)
                if vae.dtype != weight_dtype:
                    vae.to(dtype=weight_dtype)

                # ----------------------------------------------------------------------------
                # 3. encode pixel values with batch size of at most args.vae_encode_batch_size
                latents = []
                ...
                start_timesteps = solver.ddim_timesteps[index] # ddim_timesteps

                # repeat for bsz
                #custom_final_timesteps = solver.ddim_timesteps[-1]
                #custom_start_timesteps = custom_final_timesteps.repeat(bsz)
                #print(f'custom_start_timesteps = {custom_start_timesteps}')


                # index = tensor([15, 13, 39, 40, 38,  3, 46, 29, 23, 29, 22, 14], device='cuda:0')
                # start_timesteps = tensor([319, 279, 799, 819, 779,  79, 939, 599, 479, 599, 459, 299],
                # timesteps
                timesteps = start_timesteps - topk
                #print(f'index = {index} | start_timesteps = {start_timesteps} | topk = {topk} | timesteps = {timesteps}')
                # why erasing topk ?
                ...
                # [4.1] make noisy latent
                noise = torch.randn_like(latents)
                noisy_model_input = noise_scheduler.add_noise(latents,noise,start_timesteps)

                # [5.0] Sample a random guidance scale w from U[w_min, w_max]
                # Note that for LCM-LoRA distillation it is not necessary to use a guidance scale embedding
                w = (args.w_max - args.w_min) * torch.rand((bsz,)) + args.w_min # 5 ~ 15 (number twn w_min and w_max)
                ...
                noise_pred = unet(noisy_model_input,
                                  start_timesteps,
                                  timestep_cond=None, # what is timestep_cond ?
                                  encoder_hidden_states=prompt_embeds.float(),
                                  added_cond_kwargs=encoded_text,).sample
                ...
                with torch.no_grad():
                    with autocast_ctx:
                        ...
                        cond_teacher_output = teacher_unet(noisy_model_input.to(weight_dtype),
                                                           start_timesteps,
                                                           encoder_hidden_states=prompt_embeds.to(weight_dtype),).sample
                        cond_pred_x0 = get_predicted_original_sample(cond_teacher_output,
                                                                     start_timesteps,
                                                                     noisy_model_input,
                                                                     noise_scheduler.config.prediction_type,
                                                                     alpha_schedule,
                        ...
                        uncond_teacher_output = teacher_unet(noisy_model_input.to(weight_dtype),
                                                             start_timesteps,
                                                             encoder_hidden_states=uncond_prompt_embeds.to(weight_dtype),).sample
                        uncond_pred_x0 = get_predicted_original_sample(uncond_teacher_output,
                                                                       start_timesteps,
                                                                       noisy_model_input,
                                                                       noise_scheduler.config.prediction_type,
                                                                       alpha_schedule,
                                                                       sigma_schedule,)
                        uncond_pred_noise = get_predicted_noise(uncond_teacher_output,
                                                                start_timesteps,
                                                                noisy_model_input,
                                                                noise_scheduler.config.prediction_type,
                                                                alpha_schedule,
                        ...
                        # w is like classifier guidance (w = 5 ~ 15)
                        ...
                        # ddim_step function
                        x_prev = solver.ddim_step(pred_x0, pred_noise, index) # teacher model prev one step
